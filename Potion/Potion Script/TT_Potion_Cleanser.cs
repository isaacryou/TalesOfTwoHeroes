...
namespace TT.Potion
{
    public class TT_Potion_Cleanser : TT_Potion_APotionTemplate
    {
        public int potionId;
        ...
        {
            List<GameObject> allDebuffs = _playerObject.statusEffectController.GetAllExistingDebuffs(false);

            foreach (GameObject debuff in allDebuffs)
            {
                _playerObject.RemoveStatusEffect(debuff);
            }

            _playerObject.CreateBattleChangeUi(0, BattleHpChangeUiType.Normal, "", null, HpChangeDefaultStatusEffect.DebuffRemove);
