...
namespace TT.Potion
{
    public class TT_Potion_RemoveEnemyBuff : TT_Potion_APotionTemplate
    {
        public int potionId;
        ...
        public GameObject removeBuffEffect;
        ...
        {
            List<GameObject> allEnemyBuffs = _enemyObject.statusEffectController.GetAllExistingBuffs(false);

            foreach (GameObject buff in allEnemyBuffs)
            {
                _enemyObject.RemoveStatusEffect(buff);
            }

            _enemyObject.CreateBattleChangeUi(0, BattleHpChangeUiType.Normal, "", null, HpChangeDefaultStatusEffect.BuffRemove);
