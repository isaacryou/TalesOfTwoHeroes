...
namespace TT.Potion
{
    public class TT_Potion_InstaKill : TT_Potion_APotionTemplate
    {
        public int potionId;
        ...
        {
            bool enemyIsBoss = _battleController.EnemyGroupIsBoss;

            if (!enemyIsBoss)
            {
                int curEnemyHP = _enemyObject.GetCurHpValue();

                _enemyObject.TakeDamage(curEnemyHP * -1, false, false, true);

                _playerObject.TakeDamage(curEnemyHP * -1, false, false, false);

                yield return new WaitForSeconds(1.2f);

                if (_playerObject.IsObjectDead())
                {
                    _battleController.PlayerLostBattle();
                    _battleController.GetCurrentPlayer().OnPlayerDeath();
                }
                else
                {
                    _battleController.CheckAllEnemyIsDead();
                }
            }
