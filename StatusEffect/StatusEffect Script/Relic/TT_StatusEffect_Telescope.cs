...
{
    public class TT_StatusEffect_Telescope : TT_StatusEffect_ATemplate
    {
        ...
        public override void SetUpStatusEffectVariables(int _statusEffectId, Dictionary<string, string> _statusEffectVariables)
        {
            ...
            string numberOfSureHitString;
            if (_statusEffectVariables.TryGetValue("numberOfSureHit", out numberOfSureHitString))
            {
                numberOfSureHit = int.Parse(numberOfSureHitString);
            }
            else
            {
                numberOfSureHit = 0;
        ...
        public override void OnBattleStart(TT_StatusEffect_Battle _statusEffectBattle, TT_Battle_Object _battleObject, StatusEffectActionPerformed _actionTypePerformed) 
        {
            ...
            GameObject telescopeRelic = _battleObject.relicController.GetExistingRelic(relicId);
            TT_Relic_Relic relicScript = telescopeRelic.GetComponent<TT_Relic_Relic>();

            Dictionary<string, string> sureHitDictionary = new Dictionary<string, string>();
            sureHitDictionary.Add("actionCount", numberOfSureHit.ToString());

            StatusEffectInfo statusEffectInfo = new StatusEffectInfo();
            statusEffectInfo.statusEffectId = sureHitStatusEffectId;
            statusEffectInfo.statusEffectObject = sureHitStatusEffectObject;
            statusEffectInfo.statusEffectVariables = sureHitDictionary;
            ...
            _statusEffectBattle.AddStatusEffectToPerform(
                        ...
                        HpChangeDefaultStatusEffect.SureHit, //Default status effect
