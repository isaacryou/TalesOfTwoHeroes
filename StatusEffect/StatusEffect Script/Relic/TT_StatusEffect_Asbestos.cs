...
{
    public class TT_StatusEffect_Asbestos : TT_StatusEffect_ATemplate
    {
        ...
        public override void SetUpStatusEffectVariables(int _statusEffectId, Dictionary<string, string> _statusEffectVariables)
        {
            ...
            string turnDamageString;
            if (_statusEffectVariables.TryGetValue("turnDamage", out turnDamageString))
            {
                turnDamage = int.Parse(turnDamageString);
            }
            else
            {
                turnDamage = 0;
            ...
            string selfDamageString;
            if (_statusEffectVariables.TryGetValue("selfDamage", out selfDamageString))
            {
                selfDamage = int.Parse(selfDamageString);
            }
            else
            {
                selfDamage = 0;
        ...
        {
            GameObject asbestosObject = _battleObject.relicController.GetExistingRelic(relicId);
            TT_Relic_Relic asbestosScript = asbestosObject.GetComponent<TT_Relic_Relic>();
            ...
            _statusEffectBattle.AddStatusEffectToPerform(
                    ...
                    turnDamage * -1, //Amount of damage/defense/heal ; If none, pass in 0 ; If this is damage, make sure that the number is lower than 0
                    ...
                    asbestosScript, //Relic icon to pulse
        ...
        public override void OnBattleEnd(TT_StatusEffect_Battle _statusEffectBattle, TT_Battle_Object _battleObject, StatusEffectActionPerformed _actionTypePerformed) 
        {
            ...
            GameObject asbestosObject = _battleObject.relicController.GetExistingRelic(relicId);
            TT_Relic_Relic relicScript = asbestosObject.GetComponent<TT_Relic_Relic>();

            _statusEffectBattle.AddStatusEffectToPerform(
                        StatusEffectActions.OnBattleEnd, //StatusEffectAction
                        statusEffectId, //Status effect id
                        _battleObject, //Battle object
                        selfDamage * -1, //Amount of damage/defense/heal ; If none, pass in 0
                        null, //Text to show ; If none, pass in null
                        null, //Effect to play
                        BattleHpChangeUiType.Damage, //Battle HP change UI Type to determine the icon and color used to display damage/defense/heal
                        HpChangeDefaultStatusEffect.None, //Default status effect
                        ...
                        null, //Status effect to decrease turn
                        null, //Status effect to decrease action
                        true //Cannot die from this damage
                    );
        }
