...
{
    public class TT_StatusEffect_Nunchaku : TT_StatusEffect_ATemplate
    {
        ...
        public override void SetUpStatusEffectVariables(int _statusEffectId, Dictionary<string, string> _statusEffectVariables)
        {
            ...
            string battleCountString;
            if (_statusEffectVariables.TryGetValue("battleAmount", out battleCountString))
            {
                battleAmount = int.Parse(battleCountString);
            }
            else
            {
                battleAmount = 0;
            }
            string maxHealthAmountString;
            if (_statusEffectVariables.TryGetValue("maxHealthAmount", out maxHealthAmountString))
            {
                maxHealthAmount = int.Parse(maxHealthAmountString);
            }
            else
            {
                maxHealthAmount = 0;
        ...
        {
            string battleAmountString;
            if (_specialVariables.TryGetValue("currentBattleCount", out battleAmountString))
            {
                currentBattleCount = int.Parse(battleAmountString);
            }
        }
        ...
        {
            currentBattleCount++;
            if (currentBattleCount >= battleAmount)
            {
                currentBattleCount = 0;

                _battleObject.battleObjectStat.ChangeMaxHpByValue(maxHealthAmount, false);
                _battleObject.battleObjectStat.ChangeHpByValue(maxHealthAmount, BattleHpChangeUiType.Damage, false);

                TT_Player_Player playerScript = _battleObject.gameObject.GetComponent<TT_Player_Player>();
                playerScript.mainBoard.CreateBoardChangeUi(3, maxHealthAmount);
        ...
        {
            GameObject nunchakuRelic = _battleObject.relicController.GetExistingRelic(relicId);
            relicScript = nunchakuRelic.GetComponent<TT_Relic_Relic>();
        }
