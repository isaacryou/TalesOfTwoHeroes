...
{
    public class TT_StatusEffect_BeakMask : TT_StatusEffect_ATemplate
    {
        ...
        private TT_StatusEffect_Battle statusEffectBattleController;
        ...
        private bool iconIsShowing;

        void Update()
        {
            if (statusEffectBattleController == null)
            {
                return;
            }
        ...
        private void StatusEffectIsActive()
        {
            if (statusEffectBattleController == null)
            {
                return;
            ...
            int currentHp = statusEffectController.battleObject.GetCurHpValue();
            int maxHp = statusEffectController.battleObject.GetMaxHpValue();
            ...
            if (hpRatio < hpThreshold)
            {
                ...
                if (!iconIsShowing)
                {
                    statusEffectBattleController.UpdateAllStatusEffect();
                }
        ...
        {
            statusEffectBattleController = null;
        }
        ...
        {
            GameObject beakMaskRelic = _battleObject.relicController.GetExistingRelic(relicId);
            relicScript = beakMaskRelic.GetComponent<TT_Relic_Relic>();

            statusEffectBattleController = _statusEffectBattle;
        }
        ...
        public override string GetStatusEffectDescription()
        {
            ...
            string damageIncreaseAmountPercentage = StringHelper.ColorPositiveColor(damageIncreaseAmount);
            string hpThresholdPercentage = StringHelper.ColorHighlightColor(hpThreshold);
            dynamicStringPair.Add(new DynamicStringKeyValue("damageIncreaseAmount", damageIncreaseAmountPercentage));
            dynamicStringPair.Add(new DynamicStringKeyValue("hpThreshold", hpThresholdPercentage));
