...
{
    public class TT_StatusEffect_BankStatement : TT_StatusEffect_ATemplate
    {
        ...
        private float goldAmountPercentage;
        ...
        public override void SetUpStatusEffectVariables(int _statusEffectId, Dictionary<string, string> _statusEffectVariables)
        {
            ...
            string goldAmountPercentageString;
            if (_statusEffectVariables.TryGetValue("goldAmountPercentage", out goldAmountPercentageString))
            {
                goldAmountPercentage = float.Parse(goldAmountPercentageString, StringHelper.GetCurrentCultureInfo());
            }
            else
            {
                goldAmountPercentage = 0;
            }
        ...
        {
            BoardTile currentBoardTile = _statusEffectBattle.battleController.GetCurrentBoardTile();
            BoardTileType currentBoardTileType = currentBoardTile.BoardTileType;

            if (currentBoardTileType == BoardTileType.EliteBattle || currentBoardTileType == BoardTileType.BossBattle)
            {
                TT_Player_Player playerScript = _statusEffectBattle.battleController.GetCurrentPlayer();
                int currentGold = playerScript.shopCurrency;
                int goldInterestAmount = (int)(currentGold * goldAmountPercentage);
                playerScript.PerformShopCurrencyTransaction(goldInterestAmount);

                GameObject bankStatementRelic = _battleObject.relicController.GetExistingRelic(relicId);
                TT_Relic_Relic relicScript = bankStatementRelic.GetComponent<TT_Relic_Relic>();
        ...
        public override string GetStatusEffectDescription()
        {
            ...
            string goldAmountPercentageString = (goldAmountPercentage * 100).ToString();
            dynamicStringPair.Add(new DynamicStringKeyValue("goldAmountPercentage", goldAmountPercentageString));
