...
{
    public class TT_StatusEffect_Anchor : TT_StatusEffect_ATemplate
    {
        ...
        private bool happenedInThisAction;
        ...
        public override void SetUpStatusEffectVariables(int _statusEffectId, Dictionary<string, string> _statusEffectVariables)
        {
            ...
            string stunChanceString;
            if (_statusEffectVariables.TryGetValue("stunChance", out stunChanceString))
            {
                stunChance = float.Parse(stunChanceString, StringHelper.GetCurrentCultureInfo());
            }
            else
            {
                stunChance = 0;
            ...
            string stunTimeString;
            if (_statusEffectVariables.TryGetValue("stunTime", out stunTimeString))
            {
                stunTime = int.Parse(stunTimeString);
            }
            else
            {
                stunTime = 0;
            ...
            string stunTurnString;
            if (_statusEffectVariables.TryGetValue("stunTurn", out stunTurnString))
            {
                stunTurn = int.Parse(stunTurnString);
            }
            else
            {
                stunTurn = 0;
        ...
        {
            if (happenedInThisAction)
            {
                return;
            }

            happenedInThisAction = true;

            int maxHpForEffect = (int)(_battleObject.GetMaxHpValue() * hpThreshold);
            int curHp = _battleObject.GetCurHpValue();

            if (curHp > maxHpForEffect)
            {
                return;
            }
        ...
        { 
            if (applyStun)
            {
                GameObject anchorObject = _battleObject.relicController.GetExistingRelic(relicId);
                TT_Relic_Relic anchorScript = anchorObject.GetComponent<TT_Relic_Relic>();
                ...
                if (existingNullifyDebuff != null)
                {
                    _statusEffectBattle.AddStatusEffectToPerform(
                        ...
                        anchorScript, //Relic icon to pulse
                ...
                else
                {
                    ...
                    statusEffectInfo.statusEffectId = stunStatusEffectId;
                    statusEffectInfo.statusEffectObject = stunStatusEffectObject;
