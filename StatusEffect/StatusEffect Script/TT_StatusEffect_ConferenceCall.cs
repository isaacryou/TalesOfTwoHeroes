...
{
    public class TT_StatusEffect_ConferenceCall : TT_StatusEffect_ATemplate
    {
        ...
        private float conferenceCallAttackUpAmount;
        ...
        public override void SetUpStatusEffectVariables(int _statusEffectId, Dictionary<string, string> _statusEffectVariables)
        {
            ...
            string conferenceCallAttackUpAmountString;
            if (_statusEffectVariables.TryGetValue("conferenceCallAttackUpAmount", out conferenceCallAttackUpAmountString))
            {
                conferenceCallAttackUpAmount = float.Parse(conferenceCallAttackUpAmountString, StringHelper.GetCurrentCultureInfo());
            }
            else
            {
                conferenceCallAttackUpAmount = 0;
            }
            string conferenceCallAttackUpTimeString;
            if (_statusEffectVariables.TryGetValue("conferenceCallAttackUpTime", out conferenceCallAttackUpTimeString))
            {
                conferenceCallAttackUpTime = int.Parse(conferenceCallAttackUpTimeString);
            }
            else
            {
                conferenceCallAttackUpTime = 0;
            }
        ...
        public override void OnHit(TT_StatusEffect_Battle _statusEffectBattle, TT_Battle_Object _battleObject, StatusEffectActionPerformed _actionTypePerformed) 
        {
            ...
            GameObject existingVentingDelusion = _battleObject.statusEffectController.GetExistingStatusEffect(31);
            //Dodge
            GameObject existingDodgeStatusEffect = _battleObject.statusEffectController.GetExistingStatusEffect(26);

            //If any of the dodge exists
            if (existingVentingDelusion != null || existingDodgeStatusEffect != null)
            {
                ...
                //Apply a new status
                GameObject newStatusEffect = Instantiate(attackUpStatusEffectObject, battleObjectStatusEffectSet.transform);
                TT_StatusEffect_ATemplate statusEffectTemplate = newStatusEffect.GetComponent<TT_StatusEffect_ATemplate>();

                Dictionary<string, string> statusEffectDictionary = new Dictionary<string, string>();
                statusEffectDictionary.Add("actionCount", conferenceCallAttackUpTime.ToString());
                statusEffectDictionary.Add("attackUp", conferenceCallAttackUpAmount.ToString());

                statusEffectTemplate.SetUpStatusEffectVariables(attackUpStatusEffectId, statusEffectDictionary);
            }
        }
        ...
        public override string GetStatusEffectDescription()
        {
            ...
            string damageIncreasePercentage = StringHelper.ColorPositiveColor(conferenceCallAttackUpAmount);
            dynamicStringPair.Add(new DynamicStringKeyValue("conferenceCallAttackBonus", damageIncreasePercentage));
