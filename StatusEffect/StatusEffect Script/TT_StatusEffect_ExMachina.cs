...
{
    public class TT_StatusEffect_ExMachina : TT_StatusEffect_ATemplate
    {
        ...
        private int currentDamageDropOffAmount;
        private int damageDropOffAmount;
        ...
        public override void SetUpStatusEffectVariables(int _statusEffectId, Dictionary<string, string> _statusEffectVariables)
        {
            ...
            string damageDropOffAmountString;
            if (_statusEffectVariables.TryGetValue("damageDropOffAmount", out damageDropOffAmountString))
            {
                damageDropOffAmount = int.Parse(damageDropOffAmountString);
            }
            else
            {
                damageDropOffAmount = 0;
            }

            currentDamageDropOffAmount = 0;

            attackActionName = statusEffectSerializer.GetStringValueFromStatusEffect(_statusEffectId, "attackActionName");
        ...
        public override Dictionary<string, string> GetSpecialVariables()
        {
            ...
            allSpecialVariables.Add("currentDamageDropOffAmount", currentDamageDropOffAmount.ToString());
        ...
        {
            int damageDropOffReduction = 0;

            GameObject existingNewOrder = _battleObject.statusEffectController.GetExistingStatusEffect(34);
            if (existingNewOrder != null)
            {
                TT_StatusEffect_ATemplate statusEffectTemplate = existingNewOrder.GetComponent<TT_StatusEffect_ATemplate>();
                Dictionary<string, string> specialVariable = statusEffectTemplate.GetSpecialVariables();
                string currentDamageDropOffString;
                if (specialVariable.TryGetValue("totalDropOffReduction", out currentDamageDropOffString))
                {
                    damageDropOffReduction = int.Parse(currentDamageDropOffString);
                }
                else
                {
                    damageDropOffReduction = 0;
                }
            }

            int damageToDrop = damageDropOffAmount - damageDropOffReduction;
            ...
            GameObject existingCurtainCall = _battleObject.statusEffectController.GetExistingStatusEffect(35);
            if (existingCurtainCall != null)
            {
                damageToDrop *= 2;
            }

            currentDamageDropOffAmount += damageToDrop;
        }
        ...
        public override string GetStatusEffectDescription()
        {
            ...
            dynamicStringPair.Add(new DynamicStringKeyValue("attackActionName", attackActionName));
            dynamicStringPair.Add(new DynamicStringKeyValue("damageDropOffAmount", currentDamageDropOffAmount.ToString()));
