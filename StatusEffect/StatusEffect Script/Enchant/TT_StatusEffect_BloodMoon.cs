...
{
    public class TT_StatusEffect_BloodMoon : TT_StatusEffect_ATemplate
    {
        ...
        public override void SetUpStatusEffectVariables(int _statusEffectId, Dictionary<string, string> _statusEffectVariables)
        {
            ...
            selfDamage = statusEffectSerializer.GetIntValueFromStatusEffect(statusEffectId, "selfDamage");
            hpRecover = statusEffectSerializer.GetIntValueFromStatusEffect(statusEffectId, "hpRecover");
        }
        ...
        public override void OnAttack(TT_StatusEffect_Battle _statusEffectBattle, TT_Battle_Object _battleObject, StatusEffectActionPerformed _actionTypePerformed) 
        {
            ...
            int aboveThresholdHp = (int)(maxHp * aboveHpThreshold);
            int belowThresholdHp = (int)(maxHp * belowHpThreshold);

            if (curHp >= aboveThresholdHp)
            {
                _battleObject.TakeDamage(selfDamage * -1, false, false, false, true);
            }
            else if (curHp <= belowHpThreshold)
            {
                _battleObject.HealHp(hpRecover);
            }
        }
        ...
        public override string GetStatusEffectDescription()
        {
            ...
            if (gameObject.scene.name == null)
            {
                ...
                statusEffectDescription = statusEffectSerializer.GetStringValueFromStatusEffect(126, "description");
                aboveHpThreshold = statusEffectSerializer.GetFloatValueFromStatusEffect(126, "aboveHpThreshold");
                belowHpThreshold = statusEffectSerializer.GetFloatValueFromStatusEffect(126, "belowHpThreshold");
                selfDamage = statusEffectSerializer.GetIntValueFromStatusEffect(126, "selfDamage");
                hpRecover = statusEffectSerializer.GetIntValueFromStatusEffect(126, "hpRecover");
            }
            ...
            string selfDamageColor = StringHelper.ColorNegativeColor(selfDamage);
            string hpRecoverColor = StringHelper.ColorPositiveColor(hpRecover);
            ...
            dynamicStringPair.Add(new DynamicStringKeyValue("selfDamage", selfDamageColor));
            dynamicStringPair.Add(new DynamicStringKeyValue("hpRecover", hpRecoverColor));
        ...
        public override string GetStatusEffectName()
        {
            if (gameObject.scene.name == null)
            {
                ...
                statusEffectName = statusEffectSerializer.GetStringValueFromStatusEffect(126, "name");
            }
