...
{
    public class TT_StatusEffect_Avarice : TT_StatusEffect_ATemplate
    {
        ...
        private bool goldGrantedThisBattle;

        public AudioSource avariceAudioSource;
        public List<AudioClip> allAvaraiceAudioToPlay;
        ...
        public override void SetUpStatusEffectVariables(int _statusEffectId, Dictionary<string, string> _statusEffectVariables)
        {
            ...
            goldAmount = statusEffectSerializer.GetIntValueFromStatusEffect(_statusEffectId, "goldAmount");
            goldGrantedThisBattle = false;
        }
        ...
        {
            if (_statusEffectBattle.usedEquipment.GetInstanceID() == equipmentUniqueId && !goldGrantedThisBattle)
            {
                TT_Player_Player playerScript = _battleObject.gameObject.GetComponent<TT_Player_Player>();
                playerScript.PerformShopCurrencyTransaction(goldAmount);

                AudioClip randomAudioClip = allAvaraiceAudioToPlay[Random.Range(0, allAvaraiceAudioToPlay.Count)];
                avariceAudioSource.clip = randomAudioClip;
                avariceAudioSource.Play();

                goldGrantedThisBattle = true;
            }
        }
        ...
        public override string GetStatusEffectDescription()
        {
            ...
            if (gameObject.scene.name == null)
            {
                ...
                statusEffectDescription = statusEffectSerializer.GetStringValueFromStatusEffect(95, "description");
                goldAmount = statusEffectSerializer.GetIntValueFromStatusEffect(95, "goldAmount");
            }

            string goldAmountString = StringHelper.ColorPositiveColor(goldAmount);
        ...
        public override string GetStatusEffectName()
        {
            if (gameObject.scene.name == null)
            {
                ...
                statusEffectName = statusEffectSerializer.GetStringValueFromStatusEffect(95, "name");
            }
