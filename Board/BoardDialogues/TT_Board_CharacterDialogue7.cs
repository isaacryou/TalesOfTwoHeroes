...
namespace TT.Board
{
    public class TT_Board_CharacterDialogue7 : TT_Board_ACharacterDialogueTemplate
    {
        private readonly int DIALOGUE_ID = 1898;

        private string dialogueString;

        private readonly float HP_THRESHOLD = 0.6f;
        ...
        public override bool DialogueAvailable(TT_Player_Player _currentPlayer, bool _isOnCharacterSwap)
        {
            ...
            int thresholdHp = (int)(playerMaxHp * HP_THRESHOLD);

            if (playerCurHp > thresholdHp)
            {
                return true;
            }
