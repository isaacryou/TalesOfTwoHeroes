...
{
    public class TT_Equipment_GangFire : AEquipmentTemplate
    {
        private int offenseAttackValue;
        private readonly int EQUIPMENT_ID = 57;
        ...
        public override void OnAttack(TT_Battle_Object attackerObject, TT_Battle_Object victimObject, TT_StatusEffect_Battle _statusEffectBattle)
        {
            ...
            List<EffectData> allEffects = new List<EffectData>();
            allEffects.Add(offenseEffectData);
            allEffects.Add(offenseTwoEffectData);
            allEffects.Add(offenseThreeEffectData);
            allEffects.Add(offenseFourEffectData);

            StartCoroutine(ExecuteAttack(attackerObject, victimObject, _statusEffectBattle, actionIsPlayers, allEffects));
        }

        IEnumerator ExecuteAttack(TT_Battle_Object attackerObject, TT_Battle_Object victimObject, TT_StatusEffect_Battle _statusEffectBattle, bool _isPlayerAction, List<EffectData> _allEffects)
        {
            for (int i = 0; i < offenseTime; i++)
            {
                ...
                yield return new WaitForSeconds(_allEffects[i].customEffectTime);
            }
