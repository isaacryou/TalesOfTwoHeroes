...
{
    public class TT_Equipment_ThatRabbitAimTheirNeck : AEquipmentTemplate
    {
        private readonly int EQUIPMENT_ID = 145;
        ...
        public override void OnAttack(TT_Battle_Object attackerObject, TT_Battle_Object victimObject, TT_StatusEffect_Battle _statusEffectBattle)
        {
            ...
            int currentVictimDefense = victimObject.GetCurDefenseValue();
            int finalDamage = 0;
            bool nothingHappens;
            if (currentVictimDefense == 0)
            {
            ...
            victimObject.TakeDamage(damageOutput * -1, true, false, false, false, false, true, false, true, !nothingHappens);
            ...
            StartCoroutine(AttackCoroutine(nothingHappens));
        }

        IEnumerator AttackCoroutine(bool _nothingHappens)
        {
            float waitTime = (_nothingHappens) ? nothingEffectData.customEffectTime : attackEffectData.customEffectTime;
        ...
        {
            EquipmentXMLSerializer equipmentSerializer = new EquipmentXMLSerializer();
            int dodgeTurn = equipmentSerializer.GetIntValueFromEquipment(EQUIPMENT_ID, "startDodgeTurn");
            int dodgeTime = equipmentSerializer.GetIntValueFromEquipment(EQUIPMENT_ID, "startDodgeTime");

            Dictionary<string, string> dodgeStatusEffectVariables = new Dictionary<string, string>();
            dodgeStatusEffectVariables.Add("turnCount", dodgeTurn.ToString());
            dodgeStatusEffectVariables.Add("actionCount", dodgeTime.ToString());

            _battleObject.ApplyNewStatusEffectByObject(dodgeStatusEffectObject, dodgeStatusEffectId, dodgeStatusEffectVariables);
        }
