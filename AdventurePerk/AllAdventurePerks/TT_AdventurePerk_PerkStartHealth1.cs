...
{
    public class TT_AdventurePerk_PerkStartHealth1 : TT_AdventurePerk_AdventuerPerkScriptTemplate
    {
        ...
        public override void InitializePerk(AdventurePerkXMLFileSerializer adventurePerkFile)
        {
            ...
            hpUpAmount = adventurePerkFile.GetIntValueFromAdventurePerk(adventurePerkId, "hpUpAmount");
        }

        public override void OnAdventureStart(TT_Player_Player _darkPlayer, TT_Player_Player _lightPlayer)
        {
            TT_Battle_Object darkPlayerBattleObject = _darkPlayer.playerBattleObject;
            int currentDarkPlayerMaxHp = darkPlayerBattleObject.GetMaxHpValue();
            darkPlayerBattleObject.SetMaxHpValue(currentDarkPlayerMaxHp + hpUpAmount);
            darkPlayerBattleObject.SetCurHpValue(currentDarkPlayerMaxHp + hpUpAmount);

            TT_Battle_Object lightPlayerBattleObject = _lightPlayer.playerBattleObject;
            int currentLightPlayerMaxHp = lightPlayerBattleObject.GetMaxHpValue();
            lightPlayerBattleObject.SetMaxHpValue(currentLightPlayerMaxHp + hpUpAmount);
            lightPlayerBattleObject.SetCurHpValue(currentLightPlayerMaxHp + hpUpAmount);
        }
        ...
        public override string GetPerkDescription(bool _inMiddleOfAdventure = false, TT_Board_Board _mainBoard = null)
        {
            ...
            allDynamicStringKeyValue.Add(new DynamicStringKeyValue("maxHPIncrease", hpUpAmount.ToString()));
